...
export async function executeMTCInstruction(rawUserInput) {
    try {
        ...
        } else if (wCommand.command === 'actMintCurrency' || wCommand.command === 'actExchangeCurrency') {
            executionFunction(wCommand.user, wCommand.currency, wCommand.amount, wCommand.toCurrency);
        } else {
             executionFunction();
        }
